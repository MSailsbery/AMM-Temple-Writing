...
\begin{abstract}
\noindent Using Markov chains as an approach to facility location problems is considered.
In particular, a Markov chain when coupled with realistic assumptions about a consumer-supplier network is found to be able to answers questions such as: Where should a facility be placed to have the most consumer throughput, or so that the most neglected customers can have increased ability to access it? An example construction including computational results is shown for finding the best location in the United States for a new temple to be built by The Church of Jesus Christ of Latter-Day Saints. % I'm not sure how much of this sentence should be in the abstract because although it's true, it's not really a focus of the research.  I don't know what the standard is.
