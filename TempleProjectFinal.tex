...
We assume this would increase the likelihood of a customer going to a location of business because they will not a long travel time to get to the producer's location. %Super awkward, needs revising.
Here, we assume that the limiting factor for consumers is the travel time rather than distance.
Using a Markov transition matrix, we can calculate the average amount of time spent traveling from a consumer state to a producer state. %Awkward term.
...
This would encapsulate situations such as hospitals or fire stations that desire to minimize the maximum distance from a hospital or fire station.
