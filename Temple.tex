...
There are a myriad of ways to define an optimal location, but the standard definition for median problems is a location that minimizes the average distance traveled to fulfill a demand (Owen \& Daskin, 1998). % So you're telling me that an optimal location is "a subset of median problems"?  Should it be a location that solves a subset of median problems?  Or is it a place that minimizes that average distance traveled to fulfill a demand?  Additionally, I looked (albeit briefly) through the source, but it didn't seem to include a definition of an optimal location or a discussion of the many definitions that exist.
% Darren - I edited this line for what I believe sentence meant to say, while keeping it true to the cited article.
...
\subsection{Distance Model}
Insert text here.
% I didn't know which figure we were going to need, so I included both of them.
...
\caption{Results : Euclidean Graph1} % Adds a caption
\label{fig:Results1}
\end{figure}
