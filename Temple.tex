...
In an effort to provide these members with temple access, the LDS Church has built a large number of temples across the globe and continues to build more.
In fact, at this time there are 155 temples in operation worldwide in addition to 14 temples under construction and 13 more that have been announced ("Statistics", 2017).

When constructing new temples, LDS church leaders have to evaluate which locations are most effective to maximize frequent member access.
In the field of operations research, this problem is referred to as a facility location problem.
% This is a really short paragraph that flows nicely with the first paragraph. Do we even need to explain how many temples the Church has (previous paragraph)? For me, it only seems necessary to know how many temples the Church has in the US, and that only needs mentioned when we explain the setup of our model.

Facility location problems are a subfield of operations research that generally involve a set of demands in different locations, and facilities that can fulfill those demands (Melo, Nickel, \& Saldanha-da-Gama, 2009).
The idea is to choose the optimal locations of facilities to maximize the number of demands fulfilled.
% There's got to be a better way to word that last sentence...
...
In this paper, we use similar mathematical models to solve the temple location problem of the LDS church. % similar to what?
We use a Euclidean Distance Model to find the optimal temple location by maximizing the number of church members that have access to a temple. 
Additionally, we use a Markov chain model to find the optimal temple locations by maximizing the number of church members that have more frequent access to the temple. 
