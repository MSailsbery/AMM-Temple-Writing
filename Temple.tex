...
The Distance Model is used to minimize the average distance that members have to travel to their nearest temple within the United states. 
...
There are a myriad of ways to define an optimal location, but the prevailing definition is a subset of median problems that measure the average distance traveled to fulfill a demand (Owen \& Daskin, 1998). % So you're telling me that an optimal location is "a subset of median problems"?  Should it be a location that solves a subset of median problems?  Or is it a place that minimizes that average distance traveled to fulfill a demand?  Additionally, I looked (albeit briefly) through the source, but it didn't seem to include a definition of an optimal location or a discussion of the many definitions that exist.
...
Our paper seeks to fill this gap in the literature. % What a gap.  How has no one noticed it before?
...
From the simplest definition of p-median problems arises a myriad of slight variations of it to account for the context of the problem. % I thought the p-median problem was the simplest median problem?  Are there more than one original p-median problem?
...
\node[main node] (1) {$T_{1}$};
\node[main node] (2) [below left of=1] {$S_{1}$};
...
(2) edge node [right] {0.6} (1)
(2) edge [bend right] node [left] {1.5} (8)
...
\caption{A sample graph including $7$ stakes and $3$ temples. \textbf{Note}: Edges in this figure are a measure of distance, not a measure of probability.} \label{fig:M1}
\end{figure}
% End code for graph
