...
Section \ref{sec:models} will examine three modeling approaches to the problem; namely, models based off of weighted distances, Markov chains, and ... %TODO- Decide on last model.
...
This distance is often weighted.
These weight could pertain to traffic, proximity to competitors, tolls, etc., etc.
...
From this rudimentary definition of a $p$-median problem arises a myriad of slight variations of it to account for the context of the problem. % I thought the p-median problem was the simplest median problem?  Is there more than one original p-median problem?
% TO THE COMMENT ABOVE: I didn't change this sentence much in my edits because the slight variations referred to aren't the original. The p-median is the simplest and all other median problems are more complicated versions of it. If you have any other questions feel free to let me know! Also, if the sentence still doesn't make sense I can change it! -McKell
